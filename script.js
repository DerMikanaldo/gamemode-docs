...
const grouped = {};
docs.forEach((doc) => {
  const parts = doc.modul.split("›").map((s) => s.trim());
...
function renderWelcome() {
  ...
  welcome.innerHTML = `<h1>Willkommen in der Dokumentation</h1><p>Bitte wähle ein Modul in der Navigation oder benutze die Suche.</p>`;
...
function renderCategoryContent(cat, docsList, groupId) {
  ...
  // FIX: Wenn bereits eine Sub-Seite (z.B. "Core › Jobs") angezeigt wird,
  // dann NICHT nochmal nach subcat gruppieren -> verhindert doppelte "Jobs" Überschrift.
  if (cat.includes("›")) docsList = docsList.map((d) => ({ ...d, subcat: "" }));
...
function ${doc.name}(${doc.params})`;
  ...
  ["client", "shared", "server"].forEach((type) => {
  ...
  // noSubcat brauchst du hier nicht extra rendern, weil sie bereits oben nach Type drin sind.
}
...
searchInput.addEventListener("input", function () {
  ...
  const results = docs.filter(
    (d) => d.name.toLowerCase().includes(text) || d.beschreibung?.toLowerCase().includes(text)
  );
